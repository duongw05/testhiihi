<template>
  <router-view v-slot="{ Component, route }">
    ...
    <div v-if="route.name === 'product-offer'">
                      ...
                      <el-col :xs="24" :md="12" :lg="12" class="form-item">
                        <el-form-item :size="'default'" label="Tìm kiếm nhanh" prop="barcode" style="display: inherit">
                          <el-input v-model.lazy.trim="searchQuery.querySearch" clearable
                                    ...
                                    placeholder="Tìm kiếm theo tên, mã gói cước"></el-input>
                        </el-form-item>
                      </el-col>
                ...
                <el-collapse :model-value="collapseName">
                  <el-collapse-item name="2">
                    ...
                    <el-row :gutter="24" style="margin: 0; width: 100%">
                      ...
                      <el-col :xs="24" :md="6" :lg="6" class="form-item">
                        <el-form-item :size="'default'" label="Sản phẩm" prop="province"
                                      style="display: inherit">
                          <el-select v-model="searchQuery.productId" clearable filterable
                                     placeholder="Tất cả"
                                     reserve-keyword
                                     style="width: 100%"
                          >
                      ...
                      <el-col :xs="24" :md="6" :lg="6" class="form-item">
                        <el-form-item :size="'default'" label="Loại gói cước" prop="province"
                                      style="display: inherit">
                          <el-select v-model="searchQuery.productOfferType" clearable filterable
                                     placeholder="Tất cả"
                                     reserve-keyword
                                     style="width: 100%"
                          >
                      ...
                      <el-col :xs="24" :md="6" :lg="6" class="form-item">
                        <el-form-item :size="'default'" label="Hình thức cung cấp" prop="province"
                                      style="display: inherit">
                          <el-select v-model="searchQuery.provisionType" clearable filterable
                                     placeholder="Tất cả"
                                     reserve-keyword
                                     style="width: 100%"
                          >
                      ...
                      <el-col :xs="24" :md="6" :lg="6" class="form-item">
                        <el-form-item :size="'default'" label="Mã gói cước" prop="code"
                                      style="display: inherit">
                          <el-input v-model.lazy.trim="searchQuery.code" clearable></el-input>
                        </el-form-item>
                      </el-col>
                      <el-col :xs="24" :md="6" :lg="6" class="form-item">
                        <el-form-item :size="'default'" label="Tên gói cước" prop="code"
                                      style="display: inherit">
                  ...
                  <template #title>
                    ...
                    <div style="display: flex; justify-content: right; width: 100%; margin-right: 20px">
                      <el-button :icon="Download" :loading="loading"
                                 size="default" plain @click="exportProductOffer()">Xuất báo cáo
                      </el-button>
                      <el-button :icon="Plus" size="default"
                                 color="var(--system-primary-color)"
                                 type="primary" @click="navigateProductOffer('create')">Thêm
                        mới
                      </el-button>
                    </div>
                  </template>
                  <div class="layout-container-table">
                    ...
                    >
                      <template #default="scope">
                        ...
                        <el-table-column :label="'Sản phẩm'" align="left" width="150"
                                         header-align="left" prop="productName" sortable/>
                        <el-table-column :label="'Loại gói cước'" align="left" width="150"
                                         header-align="left" prop="productOfferTypeName" sortable/>
                        <el-table-column :label="'Mã gói cước'" align="left" width="150"
                                         header-align="left" prop="code" sortable/>
                        <el-table-column :label="'Tên gói cước'" align="left" sortable min-width="200"
                                         header-align="left" prop="name"/>
                        <el-table-column :label="'Hình thức cung cấp'" align="left" min-width="150"
                                         header-align="left" prop="provisionTypeName"/>
                        ...
                        <el-table-column :label="'Người tạo'" align="center" min-width="150"
                                         header-align="left" prop="createdBy"/>
                        <el-table-column :label="'Ngày tạo'" align="center" min-width="150"
                                         header-align="center" prop="createdDate"/>
                        <el-table-column :label="'Người cập nhật'" align="center" min-width="150"
                                         header-align="left" prop="lastModifiedBy"/>
                        <el-table-column :label="'Ngày cập nhật'" align="center" min-width="150"
                                         header-align="center" prop="lastModifiedDate"/>
                            ...
                            <div class="action-container">
                              <el-tooltip content="Xem chi tiết">
                                <el-button :icon="Document"
                                           ...
                                           text
                                           type="danger"
                                           @click="navigateProductOffer('view', scope.row.productId)"
                                ></el-button>
                              </el-tooltip>
                              <el-tooltip content="Cập nhật">
                                <el-button :icon="Edit"
                                           ...
                                           text
                                           type="danger"
                                           @click="navigateProductOffer('edit', scope.row.productId)"
                                ></el-button>
                              </el-tooltip>
                              <el-tooltip content="Xóa">
                                <el-button
                                    ...
                                    type="danger"
                                    @click="removeProductOffer(scope.row)"
                                >
                                </el-button>
                              </el-tooltip>
                            </div>
...
import {deleteProductOffer, exportReportProductOffer, searchProductOffer} from "@/api/productOffer";
...
export default defineComponent({
  ...
  setup() {
    ...
    const navigateProductOffer = (action: string, id?: any) => {
      switch (action) {
        case 'create':
          router.push({name: 'product-offer-create', params: {mode: 'create'}});
          break;
        case 'view':
          router.push({name: 'product-offer-view', params: {data: id, mode: 'view'}});
          break;
        case 'edit':
          router.push({name: 'product-offer-update', params: {data: id, mode: 'edit'}});
          break;
        default:
    ...
    const fetchData = async () => {
      try {
        ...
        const {data} = await searchProductOffer(searchQuery, pagination);
    ...
    const exportProductOffer = async () => {
      event.stopPropagation();
      try {
        loading.value = true
        let axiosResponse = await exportReportProductOffer(searchQuery)
        if (axiosResponse.status === 200) {
          downloadBlob(axiosResponse.data, 'Báo cáo gói cước.xlsx');
        }
      } catch (error) {
        loading.value = false
    ...
    const removeProductOffer = (row: object) => {
      ElMessageBox({
        title: 'Xóa đối tác',
        message: h('p', null, [
          h('span', {style: 'font-size: 15px'}, "Bạn có muốn xoá gói cước " + row?.name + "? "),
        ]),
        ...
        try {
          let axiosResponse = await deleteProductOffer(row?.productId);
          ElNotification({
            title: 'Thành công',
    ...
    onMounted(() => {
      if (router.currentRoute.value.name === 'product-offer') {
        fetchData();
        fetchProduct()
